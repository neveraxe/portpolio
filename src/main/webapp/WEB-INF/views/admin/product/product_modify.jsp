							...
							<form id='modifyForm' role="form" action="/admin/product/modify" method="post" enctype="multipart/form-data">
								<div class="box-body">
									<div class="form-group">
										...
										<select class="form-control" id="mainCategory" name="cg_code_pri" style="width:30%; margin-right:10px; display: inline-block;" >
										  <option value="default">1차 카테고리 선택</option>
										  
										  <c:forEach items="${categoryList }" var="cate">
										  	<option value='<c:out value="${cate.cg_code_pk }"/>' <c:out value="${cate.cg_code_pk == productVO.cg_code_pri ? 'selected' : '' }"></c:out>>
										  		<c:out value="${cate.cg_name }"></c:out>
										  	</option>
										...
										<select class="form-control" id="subCategory" name="cg_code_pk" style="width: 30%; display: inline-block;">
										 	<option value="default">2차 카테고리 선택</option>
										 	
										 	<c:forEach items="${subCategoryList }" var="subCate">
											  	<option value='<c:out value="${subCate.cg_code_pk }"/>' <c:out value="${subCate.cg_code_pk == productVO.cg_code_pk ? 'selected' : '' }"></c:out>>
											  		<c:out value="${subCate.cg_name }"></c:out>
											  	</option>
									...
									<div class="form-group">
										<label for="exampleInputEmail1">Product Name</label> <input
											type="text" id="pdt_name" name="pdt_name" value='<c:out value="${productVO.pdt_name }"></c:out>' class="form-control"
											placeholder="Enter Product name">
										<input type="hidden" id="pdt_num_pk" name="pdt_num_pk" value='<c:out value="${productVO.pdt_num_pk }"></c:out>' class="form-control" />
									</div>
									<div class="form-group">
										<label for="exampleInputEmail1">Company</label> <input
											type="text" id="pdt_company" name="pdt_company" value='<c:out value="${productVO.pdt_company }"></c:out>' class="form-control"
									...
									<div class="form-group">
										...
										<input style="width:40%; margin-right:10px; display: inline-block;"
											type="text" id="pdt_price" name="pdt_price" value='<c:out value="${productVO.pdt_price }"></c:out>' class="form-control" 
											placeholder="Enter price" />
										<input style="width:40%; display: inline-block;"
											type="text" id="pdt_discount" name="pdt_discount" value='<c:out value="${productVO.pdt_discount }"></c:out>' class="form-control "
											placeholder="Enter discounted price" />
									</div>
									<div class="form-group">
										<label for="exampleInputPassword1">Detail</label>
										<textarea class="form-control" id="pdt_detail" name="pdt_detail" rows="8"
											placeholder="Enter ..."><c:out value="${productVO.pdt_detail }"></c:out></textarea>
									</div>

									<div class="form-group">
										<label for="exampleInputEmail1">Thumbnail Image</label> 
										<input type="file" id="file1" name="file1" class="form-control" />
										<input type="hidden" id="pdt_img" name="pdt_img" value='<c:out value="${productVO.pdt_img }"></c:out>' class="form-control" />
									</div>
									...
									<div class="form-group">
										...
										<input style="width:30%; margin-right:10px; display: inline-block;"
											type="text" id="pdt_amount" name='pdt_amount' value='<c:out value="${productVO.pdt_amount }"></c:out>' class="form-control" 
											placeholder="Enter Amount" />
										<select class="form-control" id="pdt_buy" name="pdt_buy" style="width: 15%; display: inline-block;">
				 						  <option value="Y" <c:out value="${productVO.pdt_buy == 'Y' ? 'selected' : ''}"></c:out>>Y</option>
										  <option value="N" <c:out value="${productVO.pdt_buy == 'N' ? 'selected' : ''}"></c:out>>N</option>
										</select>
										<!-- 원래 리스트상태의 페이징, 검색 정보 -->
	...
	<script>
		$(document).ready(function(){
			...
			$("#mainCategory").on("change", function(){
				...
				var url = "/admin/product/subCategoryList/" + mainCategoryCode; // 뒤에는 1000, 2000, 3000 중에 선택
...
<script id="subCGListTemplate" type="text/x-handlebars-template">
	...
	{{#each .}}
	<option value="{{cg_code_pk}}">{{cg_name}}</option>
	{{/each}}
